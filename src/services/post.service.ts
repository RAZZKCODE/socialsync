...
import { getActiveInstagramAccount } from "@/lib/instagram-accounts";
...
export async function createPost(
  ...
  let credentials = credRow.credentials as PlatformCredentials;
  const selectedInstagramAccount =
    input.platform === "instagram"
      ? getActiveInstagramAccount(credentials, input.target_account_id)
      : null;

  if (input.platform === "instagram") {
    if (!selectedInstagramAccount) {
      throw new PostServiceError(
        "No Instagram account found. Please add an Instagram account in Settings.",
        "CREDENTIALS_NOT_FOUND"
      );
    }

    credentials = {
      account_id: selectedInstagramAccount.account_id,
      access_token: selectedInstagramAccount.access_token,
    };
  }
  ...
  const { data: post, error: postError } = await supabase
    ...
    .insert({
      ...
      platform_response: selectedInstagramAccount
        ? {
            target_account_id: selectedInstagramAccount.id,
            target_account_title: selectedInstagramAccount.label,
            target_platform_account_id: selectedInstagramAccount.account_id,
          }
        : undefined,
    })
    .select()
...
export async function checkAndPublishPost(
  ...
  let credentials = credRow.credentials as PlatformCredentials;
  const postAccountId =
    typeof typedPost.platform_response?.target_account_id === "string"
      ? typedPost.platform_response.target_account_id
      : undefined;

  if (typedPost.platform === "instagram") {
    const selectedInstagramAccount = getActiveInstagramAccount(
      credentials,
      postAccountId
    );
    ...
    if (!selectedInstagramAccount) {
      throw new PostServiceError(
        "Instagram account credentials not found",
        "CREDENTIALS_NOT_FOUND"
      );
    }
