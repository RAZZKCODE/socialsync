...
export default async function SettingsPage() {
  ...
  const maskedCredentials = credentials.map((c) => {
    ...
    const maskedCreds: Record<string, unknown> = { ...creds };
    ...
    if (Array.isArray(creds.accounts)) {
      maskedCreds.accounts = creds.accounts.map((account) => {
        if (!account || typeof account !== "object") return account;

        const accountRecord = account as Record<string, string>;
        const token = accountRecord.access_token;

        return {
          ...accountRecord,
          access_token:
            typeof token === "string" && token
              ? `${token.slice(0, 8)}${"*".repeat(20)}`
              : token,
        };
      });
    }
