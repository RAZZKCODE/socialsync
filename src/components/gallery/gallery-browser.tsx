...
export function GalleryBrowser({ initialItems }: GalleryBrowserProps) {
  ...
  const handleDeleteMedia = (mediaId: string) => {
    setDisplayItems((prev) => prev.filter((item) => item.id !== mediaId));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-2 md:flex-row">
        ...
        <div className="grid grid-cols-1 gap-2 min-[360px]:grid-cols-3 md:flex md:shrink-0">
          <Button
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-3 min-[420px]:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">
          {items.map((row) => (
            <GalleryMediaCard
              key={row.id}
              row={row}
